#cython: language_level=3, boundscheck=False, wraparound=False, initializedcheck=False, cdivision=True

from mpi4py import MPI
import numpy as np

comm = MPI.COMM_WORLD
cdef int size = comm.Get_size()
cdef int rank = comm.Get_rank()
cdef int is_master = rank == 0

cdef void calc_acceleration(double[:,:] loc_acc, float G, double[:,:] loc_pos, double[:,:] masses, float softening_parameter, double[:,:] tmp_data, int n, int loc_n):
  cdef int src = (rank + 1) % size
  cdef int dest = (rank - 1 + size) % size

  tmp_data[:,:] = 0.0
  loc_acc[:,:] = 0.0

  # Compute accelerations from my particles interactions with themselves
  calc_accel_2(masses, tmp_data, loc_acc, loc_pos, loc_n, rank, loc_n, rank, n, size, G, softening_parameter)

  cdef Py_ssize_t i, loc_part
  cdef int other_proc

  # Now compute forces resulting from my particles' interactions with other processes' particles
  for i in range(1, size):
    other_proc = (rank + i) % size
    comm.Sendrecv_replace(tmp_data, dest, source=src)
    calc_accel_2(masses, tmp_data, loc_acc, loc_pos, loc_n, rank, loc_n, other_proc, n, size, G, softening_parameter)

  comm.Sendrecv_replace(tmp_data, dest, source=src)
  for loc_part in range(loc_n):
      loc_acc[loc_part][0] += tmp_data[loc_n+loc_part][0]
      loc_acc[loc_part][1] += tmp_data[loc_n+loc_part][1]
      loc_acc[loc_part][2] += tmp_data[loc_n+loc_part][2]

# Given a global index glb1 assigned to process rk1, find the next higher global index assigned to process rk2
cdef int first_index(int gbl1, int rank1, int rank2, int size):
    if rank1 < rank2:
        return gbl1 + (rank2 - rank1)
    else:
        return gbl1 + (rank2 - rank1) + size

# Convert a global particle index to a global permuted index
cdef int Global_to_local(int gbl_part, int rank, int size):
    return (gbl_part - rank) / size

# Compute the forces on particles owned by process rk1 due to interaction with particles owned by procss rk2.  Exploit the symmetry (force on particle i due to particle k) = -(force on particle k due to particle i)
# rk1 = process owning particles in pos1
# rk2 = process owning contributed particles
# p = number of processes in communicator containing processes rk1 and rk2
# loc_n1 = number of my particles in pos1
# loc_n2 = number of my particles in pos2
cdef void calc_accel_2(double[:,:] masses, double[:,:] tmp_data, double[:,:] loc_acc, double[:,:] loc_pos, int loc_n1, int rank1, int loc_n2, int rank2, int n, int size, float G, float softening_parameter):
    cdef int gbl_part1 = rank1

    cdef Py_ssize_t i, j
    cdef int gbl_part2
    cdef double inverse_separations
    cdef double dx, dy, dz

    for i in range(loc_n1):
        gbl_part2 = first_index(gbl_part1, rank1, rank2, size)
        for j in range(Global_to_local(gbl_part2, rank2, size), loc_n2):
            # calculate distance between particle pairs for x,y,z
            dx = loc_pos[j,0] - loc_pos[i,0]
            dy = loc_pos[j,1] - loc_pos[i,1]
            dz = loc_pos[j,2] - loc_pos[i,2]

            # matrix of inverse seperations cubed (1/r^3)
            inverse_separations = (dx**2 + dy**2 + dz**2 + softening_parameter**2)**(-1.5)

            # calculate accelerations for x,y,z components
            loc_acc[i,0] += G * (dx * inverse_separations) * masses[gbl_part2][0]
            loc_acc[i,1] += G * (dy * inverse_separations) * masses[gbl_part2][0]
            loc_acc[i,2] += G * (dz * inverse_separations) * masses[gbl_part2][0]
            tmp_data[loc_n2+j,0] -= G * (dx * inverse_separations) * masses[gbl_part2][0]
            tmp_data[loc_n2+j,1] -= G * (dy * inverse_separations) * masses[gbl_part2][0]
            tmp_data[loc_n2+j,2] -= G * (dz * inverse_separations) * masses[gbl_part2][0]

            gbl_part2 += size

        gbl_part1 += size

cpdef simulate(double[:,:] positions, double[:,:] masses, double[:,:] velocities, float G, int N, float delta_t, float t_max, float softening_parameter):
    params = None
    positions_by_timestep = None

    if is_master:
        params = {'positions': np.array(positions), 'velocities': np.array(velocities), 'masses': np.array(masses)}

        steps = int(np.ceil(t_max / delta_t))
        positions_by_timestep = np.zeros((N,3,steps+1))
        positions_by_timestep[:,:,0] = positions

    # Broadcast / Recive enviroment
    params = comm.bcast(params, root=0)

    # Unpack environment
    positions = params['positions']
    velocities = params['velocities']
    masses = params['masses']

    cdef float t = 0

    steps = int(np.ceil(t_max / delta_t))
    cdef int count = int(np.ceil(N / size))

    cdef int loc_n = int(N / size) # Number of particales locally
    cdef int loc_n_padded = loc_n + (N % size)
    cdef int index_from = int(loc_n * rank)
    cdef int index_to   = int(loc_n * (rank + 1))
    cdef int index_to_padded = index_to + (N % size)

    cdef double[:,:] loc_pos = positions[index_from:index_to_padded]
    cdef double[:,:] loc_vel = velocities[index_from:index_to_padded]
    cdef double[:,:] loc_acc = np.zeros((loc_n_padded, 3)).astype(np.double)
    cdef double[:,:] tmp_data = np.zeros((2*loc_n_padded, 3)).astype(np.double)

    calc_acceleration(loc_acc, G, loc_pos, masses, softening_parameter, tmp_data, N, loc_n_padded)

    cdef Py_ssize_t x, i, j
    cdef double[:,:,:] _recvbuf

    cdef double[:,:] pos_temp = np.zeros((N, 3))

    for x in range(steps):
        for i in range(loc_n_padded):
            # first kick
            loc_vel[i,0] += loc_acc[i,0] * (delta_t/2.0)
            loc_vel[i,1] += loc_acc[i,1] * (delta_t/2.0)
            loc_vel[i,2] += loc_acc[i,2] * (delta_t/2.0)

            # drift
            loc_pos[i,0] += loc_vel[i,0] * delta_t
            loc_pos[i,1] += loc_vel[i,1] * delta_t
            loc_pos[i,2] += loc_vel[i,2] * delta_t

        calc_acceleration(loc_acc, G, loc_pos, masses, softening_parameter, tmp_data, N, loc_n_padded)

        for i in range(loc_n_padded):
            # second kick
            loc_vel[i,0] += loc_acc[i,0] * (delta_t/2.0)
            loc_vel[i,1] += loc_acc[i,1] * (delta_t/2.0)
            loc_vel[i,2] += loc_acc[i,2] * (delta_t/2.0)

        t += delta_t

        recvbuf = np.empty([size, loc_n_padded, 3])
        comm.Gather(loc_pos, recvbuf)

        if is_master:
            _recvbuf = recvbuf
            for j in range(size):
                l_index_from = int(loc_n * j)
                l_index_to   = int(loc_n * (j + 1)) + (N % size)

                pos_temp[l_index_from:l_index_to] = _recvbuf[j]

            positions_by_timestep[:,:,x+1] = pos_temp

    return np.array(positions_by_timestep)
