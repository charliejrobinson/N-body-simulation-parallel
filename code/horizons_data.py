...
'''
SOURCE: https://github.com/hannorein/rebound/blob/main/rebound/horizons.py
...
class Particle():
  ...
  def __repr__(self):
    return f"Particle: x={self.x}, y={self.y}, z={self.z}, vx={self.vx}, vy={self.vy}, vz={self.vz}, m={self.m}, hash={self.hash}"
...
def getParticle(particle=None, m=None, x=None, y=None, z=None, vx=None, vy=None, vz=None, primary=None, a=None,
    ...
    print(f"Searching NASA Horizons for '{particle}' on {date}...")
    idn = None
    ...
    if m is not None:
        ...
        try:
            ...
            warnings.warn("Warning: masses cannot be retrieved from NASA HORIZONS. Set to 0.", RuntimeWarning)
            p.m = 0
    else:
        warnings.warn("Warning: masses cannot be retrieved from NASA HORIZONS. Set to 0.", RuntimeWarning)
        p.m = 0
...
# There is currently no way to get masses data from HORIZONS.
...
# TODO let user pick date
def get_bodies(names=None, max_bodies=50, date=None):
  if names is None:
    names = []

    for line in HORIZONS_MASS_DATA.split('\n'):
      if line.strip() == '':
        continue

      id = line.split('=')[0].strip().split('_')[0].split('BODY')[1]
  ...
  bodies = []
  for id in names[:max_bodies]:
    bodies.append(getParticle(id, date=date))
  
  # Reformat
  masses = np.array([[b.m] for b in bodies]).astype(np.double) # in kg
  positions = np.array([[b.x, b.y, b.z] for b in bodies]).astype(np.double) * 1000 # convert from km to m
  velocities = np.array([[b.vx, b.vy, b.vz] for b in bodies]).astype(np.double) * 1000 # Convert from km/s to m/s
